...
int get_avail_ino() {
	...
	for (i = 0; i < MAX_INUM; i++) {
		...
		if (get_bitmap(inodeBitmap, i) == 0) break;
...
int get_avail_blkno() {
	...
	for (i = 0; i < MAX_DNUM; i++) {
		...
		if (get_bitmap(dataBitmap, i) == 0) break;
...
int rufs_mkfs() {
	...
	dataBitmap = (bitmap_t)malloc(BLOCK_SIZE) ;
...
static int rufs_getattr(const char *path, struct stat *stbuf) {
	...
	if (get_node_by_path(path, 0, i) != 0) {
...
static int rufs_create(const char *path, mode_t mode, struct fuse_file_info *fi) {
	...
	ustat->st_mode = __S_IFREG | 0666 ; // File
	ustat->st_nlink = 1 ;
...
static int rufs_unlink(const char *path) {
	...
	for (i = 0 ; i < 8 ; i++) { // Large file support
		...
		unset_bitmap(dataBitmap, target->indirect_ptr[i] - superblock->d_start_blk) ;
		...
		for (j = 0 ; j < (BLOCK_SIZE / sizeof(int)) ; j++) {
			...
			if (*a == 1) {
				...
				unset_bitmap(dataBitmap, *a - superblock->d_start_blk) ;
